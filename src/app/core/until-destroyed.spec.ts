...
fdescribe('untilDestroyed', () => {
  fit('should not destroy other instances', () => {
    // Arrange
  ...
  fit('should work with multiple observables', () => {
    // Arrange
    const spy = createObserver();
    const spy2 = createObserver();
  ...
  fit('should work with classes that are not components', () => {
    // Arrange
    const spy = createObserver();
  ...
  fit('should unsubscribe from anywhere', () => {
    // Arrange
    const spy = createObserver();
    const spy2 = createObserver();
    const spy3 = createObserver();
  ...
  fit('should throw when destroy method doesnt exist', () => {
    // Arrange
    const spy = createObserver();
  ...
  fit('should not throw when destroy method is implemented on super class', () => {
    // Arrange
    const spy = createObserver();
